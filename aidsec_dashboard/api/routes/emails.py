...
def list_drafts(
    ...
    """Fetch all pending email drafts generated by AI. Optional filters: kategorie, campaign_id, search (lead name/firma)."""
    from sqlalchemy.orm import joinedload
    from sqlalchemy import or_
    q = (
        db.query(EmailHistory)
        .options(joinedload(EmailHistory.lead), joinedload(EmailHistory.campaign))
        .filter(EmailHistory.status == EmailStatus.DRAFT)
    )
    if kategorie or search:
        q = q.join(EmailHistory.lead)
    if kategorie:
        q = q.filter(Lead.kategorie == kategorie)
    if campaign_id is not None:
        q = q.filter(EmailHistory.campaign_id == campaign_id)
    if search:
        search_term = f"%{search}%"
        q = q.filter(or_(Lead.firma.ilike(search_term), Lead.name.ilike(search_term)))
    rows = q.order_by(EmailHistory.id.desc()).all()
    return [
        GlobalEmailHistoryOut(
            id=r.id,
            lead_id=r.lead_id,
            lead_firma=r.lead.firma if r.lead else None,
            lead_kategorie=(
                r.lead.kategorie.value if r.lead and r.lead.kategorie and hasattr(r.lead.kategorie, "value")
                else (str(r.lead.kategorie) if r.lead and r.lead.kategorie else None)
            ),
